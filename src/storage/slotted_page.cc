...
void SlottedPage::compact_() {
    ...
    for (size_t offset = metadata_size(); offset < PAGE_SIZE;) {
        ...
        if (offset != cursor) {
            std::memmove(page_data_.get() + cursor,
                         page_data_.get() + offset,
                         seg_len);
...
void SlottedPage::print() const {
    ...
    std::cout << "free bytes: "
              << PAGE_SIZE - metadata_size() - used_bytes_(slots)
              << std::endl;
    std::cout << std::endl;
    for (size_t i = 0; i < MAX_SLOTS; ++i) {
        ...
        if (!s.empty) {
            ...
            std::cout << "Slot " << i << " : [" << s.offset << "] : [" << s.length << "] ::";
            if (loaded)
                loaded->print();
            std::cout << std::endl;
        }
